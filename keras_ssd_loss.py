...
class SSDLoss:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        loc_true  =  y_true[:,:, :4] # Ground truth prediction
        ...
        classification_loss = tf.to_float(self.log_loss(conf_true, conf_pred))     # Output shape: (batch_size, n_boxes)
        localization_loss   = tf.to_float(self.smooth_L1_loss(loc_true, loc_pred)) # Output shape: (batch_size, n_boxes)

        # 2: Compute the classification losses for the positive and negative targets.

        # Create masks for the positive and negative ground truth classes.
        negatives = conf_true[:,:,0] # Tensor of shape (batch_size, n_boxes), No. of samples with background  class
        positives = tf.to_float(tf.reduce_max(conf_true[:,:,1:], axis=-1)) # Tensor of shape (batch_size, n_boxes)
